...
function RewardsPage() {
  ...
  async function submit(kind: "claim" | "withdraw" | "all") {
    ...
    try {
      ...
      toast.success(kind === "claim" ? "Holder rewards claimed." : "TraffIQ withdrawal confirmed.");
  ...
  async function claimRoundReward(roundNumber: bigint) {
    ...
    try {
      ...
      await wallet.provider.waitForTransactionReceipt({ hash });
      toast.success(`Round #${roundNumber.toString()} TraffIQ was added to your staking position.`);
      await refresh();
      await betting.readClaimState();
    } catch (error) {
      toast.error((error as Error).message || "Unable to claim this round's TraffIQ reward.");
    } finally {
      setClaimingRound(null);
    }
  }

  const traffiq = (value: bigint) => `${formatUnits(value, 18)} TraffIQ`;
  const validShares = preview[0] > 0n && !!sharesInput;
  return <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
    <p className="label-tech">Rewards center</p><h1 className="mt-3 text-4xl sm:text-5xl">TraffIQ Rewards</h1>
    <p className="mt-4 max-w-2xl text-muted-foreground">Round TraffIQ rewards are automatically staked when claimed. Wallet TraffIQ and staked TraffIQ remain separate.</p>
    {!wallet.connected || !wallet.isCorrectNetwork ? <Panel className="mt-8"><p className="font-mono text-sm text-destructive">{wallet.connected ? "Wrong Network — switch to the supported network." : "Connect a wallet to view your rewards."}</p></Panel> : <>
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[["Wallet TraffIQ", traffiq(position.wallet)], ["Staked TraffIQ", traffiq(position.gross)], ["Staking Shares", formatUnits(position.shares, 18)], ["Pending Holder Rewards", traffiq(position.pending)]].map(([label, value]) => <Panel key={label}><p className="label-tech">{label}</p><p className="mt-3 font-mono text-xl text-primary">{value}</p></Panel>)}
      </div>
